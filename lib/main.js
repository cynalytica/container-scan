...
function run() {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        yield Promise.all(images.map(runImage));
        yield Promise.all([(0, utils_1.concatSarifs)(), (0, utils_1.createHtmlOutput)()]);
    });
}
exports.run = run;
function runImage(image) {
    return __awaiter(this, void 0, void 0, function* () {
        yield Promise.allSettled([runImageSarif(image), inputHelper.isRunIssueCreateEnabled() && runImageIssue(image), runImageAudit(image)]);
    });
}
function runImageSarif(image) {
    return __awaiter(this, void 0, void 0, function* () {
        const { status } = yield trivyHelper.runTrivyTemplate(image, sarif_1.SARIFTemplate, trivyHelper.getTrivySarifOutputPath(image));
...
function runImageAudit(image) {
    return __awaiter(this, void 0, void 0, function* () {
        const { status } = yield trivyHelper.runTrivyTemplate(image, table_1.HTMLTableTemplate, trivyHelper.getTrivyHtmlOutputPath(image));
        if (status === trivyHelper.TRIVY_EXIT_CODE) {
            const vulns = trivyHelper.getFilteredOutput(image);
            core.info(`Vulnerabilities were detected in the container ${image} ${vulns.length}`);
        }
...
function runImageIssue(image) {
    return __awaiter(this, void 0, void 0, function* () {
        const { status } = yield trivyHelper.runTrivy(image);
        if (status === trivyHelper.TRIVY_EXIT_CODE) {
            //create issues here?!
            const vulns = trivyHelper.getFilteredOutput(image);
